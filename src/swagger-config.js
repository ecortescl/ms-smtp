...
const options = {
  definition: {
    ...
    servers: (() => {
      ...
      // Server principal basado en variables de entorno
      const protocol =
        process.env.SWAGGER_PROTOCOL ||
        (process.env.NODE_ENV === "production" ? "https" : "http");
      const host = process.env.SWAGGER_HOST || process.env.HOST || "localhost";
      const port = process.env.SWAGGER_PORT || process.env.PORT || "3000";
      const basePath = process.env.SWAGGER_BASE_PATH?.replace(/\/+$/, "") || ""; // Remove trailing slashes

      // Construir URL principal
      let mainUrl = `${protocol}://${host}`;

      // Solo agregar puerto si no es el puerto por defecto para el protocolo
      ...
      // Servidor adicional si se especifica
      if (process.env.SWAGGER_ADDITIONAL_SERVER_URL) {
        servers.push({
          url: process.env.SWAGGER_ADDITIONAL_SERVER_URL,
          description:
            process.env.SWAGGER_ADDITIONAL_SERVER_DESCRIPTION ||
            "Additional server",
        });
      }

      // Fallback si no hay servidores configurados
      if (servers.length === 0) {
        servers.push({
        ...
        CreateTemplateRequest: {
          ...
          properties: {
            id: {
              ...
              description:
                "Optional custom template ID (alphanumeric, 3-64 chars)",
            },
            name: {
              type: "string",
              minLength: 1,
            ...
            defaults: {
              ...
              description:
                "Default values for template variables and email fields",
            },
          },
        },
